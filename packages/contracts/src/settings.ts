...
export const GitHubCopilotSettings = makeProviderSettingsSchema(
  {
    ...
    launchArgs: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "ACP server arguments",
        description: "Additional arguments passed to Copilot CLI when starting its ACP server.",
        providerSettingsForm: {
          placeholder: "e.g. --experimental --reasoning-effort=high",
          clearWhenEmpty: "omit",
        },
      }),
    ),
